...
- `[general] token_env` can point to an environment variable containing the Discord token. `[general] token = env:VARIABLE_NAME` is also accepted. Plaintext token values in `config.ini` are ignored and cleared by the setup dialog.
