...
The build embeds one version into both EXEs, the generated AppX manifest, startup diagnostics, the tray menu, and `--version`. By default the script refreshes and prunes tags from `origin`, reads the `DesktopStub-vN` Git tag family, and builds the next package version as `N.0.0.0`; for example, after `DesktopStub-v17`, an untagged local/CI build reports `DesktopStub-v18 (18.0.0.0)`. A missing Git installation, missing `origin`, or failed network refresh is non-fatal and falls back to local tags. Set `DESKTOPSTUB_SKIP_TAG_REFRESH=1` for a deliberately offline build, or set `DESKTOPSTUB_VERSION=18.0.0.0` / `DESKTOPSTUB_RELEASE_TAG=DesktopStub-v18` to use an explicit custom version without fetching.
