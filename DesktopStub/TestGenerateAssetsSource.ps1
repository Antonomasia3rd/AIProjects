...
$sourceChecks += @(
    (New-SourceCheck `
        -Name 'Live wallpaper capture rejects blank PrintWindow frames' `
        -SourceName 'src\ga_wallpaper.inc' `
        -SourceText (Read-Source 'src\ga_wallpaper.inc') `
        -Pattern 'PixelsLookBlankOrBlack' `
        -Failure 'live wallpaper capture must reject black/blank snapshots instead of generating black tiles'),

    (New-SourceCheck `
        -Name 'Live wallpaper capture skips desktop icon WorkerW' `
        -SourceName 'src\ga_wallpaper.inc' `
        -SourceText (Read-Source 'src\ga_wallpaper.inc') `
        -Pattern 'SHELLDLL_DefView.*desktop icon layer' `
        -Failure 'live wallpaper capture must skip the WorkerW that owns desktop icons'),

    (New-SourceCheck `
        -Name 'Live wallpaper capture waits for a real host window' `
        -SourceName 'src\ga_wallpaper.inc' `
        -SourceText (Read-Source 'src\ga_wallpaper.inc') `
    ...
    (New-SourceCheck `
        ...
        -SourceName 'src\ga_wallpaper.inc' `
        -SourceText (Read-Source 'src\ga_wallpaper.inc') `
        -Pattern 's_captureSequence' `
        -Failure 'live wallpaper recaptures should use bounded rotating paths for changed snapshots'),

    (New-SourceCheck `
        -Name 'Live wallpaper startup settle does not publish every warm-up capture' `
        -SourceName 'src\ga_wallpaper.inc' `
        -SourceText (Read-Source 'src\ga_wallpaper.inc') `
        -Pattern 'Do not publish the warm-up snapshots one by one' `
    ...
    (New-SourceCheck `
        ...
        -Pattern 'LiveWallpaperCaptureRefreshMs", L"10000' `
        -Failure 'LiveWallpaperCaptureRefreshMs should default to 10000 ms'),

    (New-SourceCheck `
        -Name 'Live wallpaper startup recapture duration defaults off' `
        -SourceName 'src\ga_config_defaults.inc' `
        -SourceText (Read-Source 'src\ga_config_defaults.inc') `
        -Pattern 'LiveWallpaperCaptureStartupRefreshDurationMs", L"0' `
        -Failure 'LiveWallpaperCaptureStartupRefreshDurationMs should default to 0/off'),

    (New-SourceCheck `
        -Name 'Live wallpaper pending blocks static fallback' `
        -SourceName 'src\ga_wallpaper.inc' `
        -SourceText (Read-Source 'src\ga_wallpaper.inc') `
        -Pattern 'g_liveWallpaperCapturePending' `
        -Failure 'live wallpaper startup should be able to defer fallback to the static wallpaper until capture settles'),

    (New-SourceCheck `
        -Name 'Live wallpaper screen fallback is opt-in' `
        -SourceName 'src\ga_wallpaper.inc' `
        -SourceText (Read-Source 'src\ga_wallpaper.inc') `
        -Pattern 'LiveWallpaperCaptureScreenFallbackEnabled' `
        -Failure 'screen-DC fallback must be opt-in because it can capture the visible desktop')
)
