...
int main(int argc, char** argv)
{
    ...
    try
    {
        ...
        checks.push_back({"DesktopStub tray log buffer uses shared recent-log model", "DesktopStub logging sources", loggingCore + "\n" + ReadSource("src\\ga_ui_state.inc"), R"rx(aip::RecentLogBuffer\s+g_logBuf.*g_logBuf\.SetMaxLines.*g_logBuf\.Push.*g_logBuf\.Snapshot\(\))rx", "DesktopStub should use the shared recent-log buffer while keeping product-specific logging text and console behavior", false});
