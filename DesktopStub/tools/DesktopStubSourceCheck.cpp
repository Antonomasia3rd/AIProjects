...
int main(int argc, char** argv)
{
    ...
    try
    {
        ...
        const std::string liveTileTemplates = ReadSource("src\\ga_live_tile_templates.inc");
        ...
        checks.push_back({"DesktopStub version is visible in CLI", "src\\ga_command_line.inc", commandLine, R"rx(--version.*CommandLineVersionText.*DesktopStubVersionDisplayText.*CommandLineShouldShowVersion)rx", "DesktopStub --version must report the release tag and package version", false});
        checks.push_back({"DesktopStub version is visible in the baseline tray header", "src\\ga_tray_menu.inc", tray, R"rx(versionLine.*DesktopStubVersionDisplayText.*AppendBaselineTrayMenuHeader.*versionLine)rx", "DesktopStub tray root must pass its release version through the shared baseline header", false});
        ...
        checks.push_back({"DesktopStub uses shared command-line primitives", "src\\ga_command_line.inc", commandLine, R"rx(using CommandLineIniSetting = aip::IniSetting.*aip::IsOptionOrInlineValue.*aip::TakeCommandLineValue.*aip::ParseBoolValue.*aip::ParseIniSetSpec)rx", "DesktopStub must retain its product-specific option semantics while consuming the shared command-line primitives", false});
        ...
        checks.push_back({"DesktopStub acknowledges handled instance requests", "src\\ga_app.inc", app, R"rx(HandleSingleInstanceRequest\(w\);\s*return aip::INSTANCE_REQUEST_HANDLED;)rx", "single-instance senders must not report success when the target window did not recognize the request", false});
        checks.push_back({"DesktopStub derives sidecar paths through shared helper", "src\\ga_app.inc", app, R"rx(aip::TryBuildSidecarPathsFromExecutable)rx", "DesktopStub startup path derivation must use the shared sidecar path helper", false});
        checks.push_back({"DesktopStub preserves executable-side default log paths", "src\\ga_manifest.inc", manifest, R"rx(DefaultLogPath\(\).*BuildExecutableSidecarLogPath\(g_exePath,\s*L"DesktopStub"\))rx", "DesktopStub empty LogPath defaults should remain beside the executable even when --ini points elsewhere", false});
        checks.push_back({"DesktopStub single-instance identity uses shared path-scoped helper", "src\\ga_app.inc", app, R"rx(aip::BuildPathScopedInstanceIdentity\()rx", "DesktopStub single-instance naming must use the shared path-scoped identity helper", false});
        checks.push_back({"DesktopStub log appends use shared UTF-8 append primitive", "src\\ga_core.inc", core, R"rx(aip::AppendUtf8TextToFile\(path, text, true, lockWaitMs\))rx", "DesktopStub log append compatibility must be provided by the shared logging helper", false});
        checks.push_back({"DesktopStub tray log buffer uses shared recent-log model", "DesktopStub logging sources", ReadSource("src\\ga_ui_state.inc") + "\n" + loggingCore, R"rx(aip::RecentLogBuffer\s+g_logBuf.*g_logBuf\.SetMaxLines.*g_logBuf\.Push.*g_logBuf\.Snapshot\(\))rx", "DesktopStub should use the shared recent-log buffer while keeping product-specific logging text and console behavior", false});
        checks.push_back({"DesktopStub tray wrapper splits helpers, menu, and dispatch", "src\\ga_tray.inc", trayWrapper, R"rx(#include "ga_tray_helpers\.inc"\s*#include "ga_tray_menu\.inc"\s*#include "ga_tray_dispatch\.inc")rx", "DesktopStub tray should be split into helper/menu/dispatch modules instead of one monolithic file", false});
        checks.push_back({"Live Tile preset catalog is split from transport code", "src\\ga_live_tile.inc", liveTile, R"rx(#include "ga_live_tile_templates\.inc")rx", "preset catalog selection should remain isolated from package activation and update transport code", false});
        ...
        checks.push_back({"Obsolete Manifest INI section has no remover", "src\\ga_config_defaults.inc", defaults, R"rx(RemoveObsoleteManifestIniSettings)rx", "obsolete user configuration must be left untouched instead of being deleted during startup", true});
        ...
        checks.push_back({"Effective INI uses separate single-instance scope", "src\\ga_app.inc", app, R"rx(ConfigureSingleInstanceIdentity\(const std::wstring& exeDir,\s*const std::wstring& iniPath,\s*bool customIniPath\).*Scope by the effective INI path.*!iniPath\.empty\(\)\s*\?\s*iniPath\s*:\s*exeDir.*aip::BuildPathScopedInstanceIdentity.*ConfigureSingleInstanceIdentity\(dir,\s*g_iniPath,\s*g_commandLine\.customIniPath\))rx", "renamed EXE/default INI and alternate --ini runs must not signal each other unless they share the same INI", false});
        ...
        checks.push_back({"Explicit single-instance commands override ordinary launch policy", "src\\ga_app.inc", app, R"rx(bool explicitSingleInstanceRequest = false;.*CommandLineShouldExit\(\).*explicitSingleInstanceRequest = true;.*CommandLineShouldGenerateNow\(\).*explicitSingleInstanceRequest = true;.*CommandLineShouldRegenerateManifest\(\).*explicitSingleInstanceRequest = true;.*CommandLineHasPersistentSettings\(\).*explicitSingleInstanceRequest = true;.*TryGetAlreadyRunningRequest)rx", "explicit exit, generate, manifest, and persistent-setting requests must retain precedence over AlreadyRunningAction", false});
        ...
        checks.push_back({"Main message-loop failures are handled", "src\\ga_app.inc", app, R"rx(if \(messageResult == -1\)\s*\{\s*LogWin32Failure\(L"GetMessageW", GetLastError\(\)\);\s*messageLoopExitCode = 1;)rx", "GetMessageW failure must not dispatch an uninitialized MSG or report success", false});
        checks.push_back({"Main message-loop status reaches process exit", "src\\ga_app.inc", app, R"rx(return messageLoopExitCode;)rx", "DesktopStub must return nonzero when its main message queue fails", false});
        checks.push_back({"Console control handler only acknowledges delivered shutdown requests", "src\\ga_app.inc", app, R"rx(if \(!h\)\s*return TRUE;\s*if \(PostMessageW\(h,\s*WM_APP_REQUEST_SHUTDOWN,\s*ctrlType,\s*0\)\)\s*return TRUE;\s*g_consoleControlShutdownPending = false;\s*return FALSE;)rx", "a failed console-control PostMessage must fall through to the next handler instead of silently swallowing process shutdown", false});
        checks.push_back({"AppX host detection requires an exact queried image path", "src\\ga_app.inc", app, R"rx(if \(!QueryProcessImagePath\(pe\.th32ProcessID,\s*imagePath\)\s*\|\|\s*!SamePathI\(imagePath,\s*hostPath\)\)\s*continue;)rx", "a same-named inaccessible process must not suppress launching the configured AppX host", false});
        checks.push_back({"AppX host image-path queries grow beyond MAX_PATH", "src\\ga_app.inc", app, R"rx(QueryProcessImagePath.*std::vector<wchar_t> buffer\(512\).*ERROR_INSUFFICIENT_BUFFER.*32768)rx", "long host paths must not be treated as query failures or name-only matches", false});
        ...
        checks.push_back({"Wallpaper fallback uses dynamic roaming AppData lookup", "src\\ga_wallpaper.inc", wallpaper, R"rx(SHGetKnownFolderPath\(FOLDERID_RoamingAppData.*CoTaskMemFree\(appData\))rx", "long roaming-profile paths must not disable the TranscodedWallpaper fallback", false});
        checks.push_back({"Live wallpaper snapshots use shared growable temp-path lookup", "src\\ga_wallpaper.inc", wallpaper, R"rx(aip::GetTemporaryDirectoryPath\(\))rx", "long TEMP paths must not silently redirect snapshots beside the executable", false});
        ...
        checks.push_back({"Provider-specific live wallpaper settings were removed", "source tree", JoinSource({"src\\ga_config_defaults.inc","src\\ga_command_line.inc","src\\ga_tray.inc","src\\ga_tray_helpers.inc","src\\ga_tray_menu.inc","src\\ga_tray_dispatch.inc","src\\ga_ui_strings.inc","src\\ga_ui_state.inc"}), R"rx(LiveWallpaperCaptureLively|LiveWallpaperCaptureWallpaperEngine|live-wallpaper-lively|live-wallpaper-wallpaper-engine|LiveWallpaperProviderLively|LiveWallpaperProviderWallpaperEngine)rx", "legacy provider-specific live wallpaper settings/strings/command-line switches should stay removed", true});
        checks.push_back({"Legacy named live wallpaper UI was removed", "source tree", JoinSource({"README.md","src\\ga_config_defaults.inc","src\\ga_command_line.inc","src\\ga_tray.inc","src\\ga_tray_helpers.inc","src\\ga_tray_menu.inc","src\\ga_tray_dispatch.inc","src\\ga_ui_strings.inc","src\\ga_ui_state.inc","src\\ga_wallpaper.inc"}), R"rx(Capture generic WorkerW live wallpaper|Generic live wallpaper|Generic=|LiveWallpaperSnapshot_Generic|LiveWallpaperCaptureGeneric|LiveWallpaperProviderGeneric|live-wallpaper-generic)rx", "after provider-specific live wallpaper capture was removed, the remaining WorkerW detector should be presented as the normal live wallpaper detector", true});
