...
int main(int argc, char** argv)
{
    ...
    try
    {
        ...
        checks.push_back({"DesktopStub consumes shared application subsystems", "DesktopStub baseline sources", desktopStub + "\n" + app + "\n" + commandLine + "\n" + tray, R"rx(dependencies\\desktop_app_baseline\.h.*aip::BuildPathScopedInstanceIdentity.*aip::SignalInstanceWindow.*aip::RegisterTaskbarCreatedMessage.*aip::FormatTextTemplate.*aip::TraySectionLayout)rx", "DesktopStub must be a consumer of the reusable baseline rather than the source of private implementations", false});
        ...
        checks.push_back({"DesktopStub uses shared tray menu primitives", "DesktopStub tray sources", tray, R"rx(aip::AppendTrayMenuItem.*aip::BeginTrayNestedMenu.*aip::EndTrayNestedMenu)rx", "DesktopStub must consume the shared tray item and submenu helpers while retaining its product-specific menu layout", false});
        ...
        checks.push_back({"DesktopStub smoke verifies binary, manifest, and concurrent INI behavior", "..\\.github\\tools\\RepoTools.cs", repoTools, R"rx(VerifyDesktopStubBinaryVersion\(sourceExe.*DesktopStubLiveTileBroker\.exe.*helper version does not match.*--version.*VerifyDesktopStubConcurrentIniWrites\(exe,\s*concurrentIni\).*VerifyDesktopStubManifestVersion\(manifestAfterSmoke,\s*desktopStubVersion\))rx", "DesktopStub smoke tests must verify Win32 file/product versions, broker version parity, --version, concurrent INI writes, and AppxManifest.xml version parity", false});
