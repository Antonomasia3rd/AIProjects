...
int main(int argc, char** argv)
{
    ...
    try
    {
        ...
        checks.push_back({"DesktopStub consumes shared application subsystems", "DesktopStub baseline sources", desktopStub + "\n" + app + "\n" + commandLine + "\n" + tray, R"rx(dependencies\\baseline_app\.h.*aip::BuildInstanceIdentity.*aip::SignalInstanceWindow.*aip::RegisterTaskbarCreatedMessage.*aip::FormatTextTemplate.*aip::TraySectionLayout)rx", "DesktopStub must be a consumer of the reusable baseline rather than the source of private implementations", false});
        checks.push_back({"DiscordRPC consumes shared application subsystems", "DiscordRPC baseline sources", discordMain + "\n" + discordCore + "\n" + discordApp + "\n" + discordDefaults + "\n" + discordTray, R"rx(dependencies\\baseline_app\.h.*aip::IniConfigStore.*aip::BuildInstanceIdentity.*aip::SignalInstanceWindow.*aip::RegisterTaskbarCreatedMessage.*aip::FormatTextTemplate.*aip::TraySectionLayout)rx", "DiscordRPC must use the same application subsystem contracts as DesktopStub", false});
