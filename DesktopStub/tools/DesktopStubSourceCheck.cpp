...
int main(int argc, char** argv)
{
    ...
    try
    {
        ...
        checks.push_back({"DesktopStub tray presentation is separated from command dispatch", "src\\ga_tray.inc", tray, R"rx(static void DispatchTrayCommand\(HWND h, UINT cmd\);.*void Menu\(HWND h\).*DispatchTrayCommand\(h, cmd\);\s*\}\s*static void DispatchTrayCommand\(HWND h, UINT cmd\)\s*\{\s*switch\(cmd\))rx", "DesktopStub Menu must build and display the tray while a dedicated dispatcher owns command behavior", false});
