...
int main(int argc, char** argv)
{
    ...
    try
    {
        ...
        checks.push_back({"DiscordRPC backend waits are interruptible", "DiscordRPC backend/app sources", discordApp + "\n" + discordIpc + "\n" + discordGateway, R"rx(static bool BackendControlRequested\(\).*DiscordIpcClient\(clientId,\s*verbose,\s*LoadIpcOptions\(\),\s*BackendControlRequested\).*DiscordGatewayClient\(token,\s*clientId,\s*verbose,\s*LoadGatewayOptions\(\),\s*BackendControlRequested\).*Discord IPC operation cancelled.*CancellationRequested\(\).*Gateway connection cancelled)rx", "resident exit, reload, and refresh must interrupt IPC and Gateway waits instead of waiting for their full configured timeout", false});
