...
int main(int argc, char** argv)
{
    ...
    try
    {
        ...
        checks.push_back({"Command-line help is INI-backed", "command-line/config sources", commandLine + "\n" + defaults + "\n" + uiSources, R"rx(FormatCommandLineHelpTemplate.*DecodeCommandLineHelpEscapes.*CommandLineDefaultUsageText.*g_commandLineHelpDefaults.*CommandLineUsageText.*IniReadS\( COMMAND_LINE_HELP_SECTION, COMMAND_LINE_HELP_TEMPLATE_KEY, g_commandLineHelpDefaults\[0\]\.value\).*COMMAND_LINE_HELP_SECTION.*g_commandLineHelpDefaults.*EnsureIniDefaults.*g_commandLineHelpDefaults)rx", "command-line help text must stay configurable from [CommandLineHelp] Template while --help can use the built-in default template without side effects", false});
        checks.push_back({"Command-line help is side-effect-free", "src\\ga_app.inc", app, R"rx(if \(g_commandLine\.helpRequested\).*ShowCommandLineMessage\(CommandLineDefaultUsageText\(\), false\))rx", "--help must use the built-in default help template without creating, normalizing, or repairing the INI", false});
