...
int main(int argc, char** argv)
{
    ...
    try
    {
        ...
        const std::string sharedCore = ReadSource("..\\dependencies\\core.inc");
        const std::string sharedCommandLine = ReadSource("..\\dependencies\\command_line.inc");
        ...
        const std::string baselineApp = ReadSource("..\\dependencies\\baseline_app.h");
        const std::string desktopAppBaseline = ReadSource("..\\dependencies\\desktop_app_baseline.h");
        const std::string discordCore = ReadSource("..\\DiscordRPC\\src\\drpc_core.inc");
        const std::string discordCommandLine = ReadSource("..\\DiscordRPC\\src\\drpc_command_line.inc");
        const std::string discordTypes = ReadSource("..\\DiscordRPC\\src\\drpc_types.inc");
        const std::string discordTray = ReadSource("..\\DiscordRPC\\src\\drpc_tray.inc");
        const std::string discordDefaults = ReadSource("..\\DiscordRPC\\src\\drpc_config_defaults.inc");
        const std::string discordApp = ReadSource("..\\DiscordRPC\\src\\drpc_app.inc");
        const std::string discordIpc = ReadSource("..\\DiscordRPC\\src\\drpc_ipc.inc");
        const std::string discordGateway = ReadSource("..\\DiscordRPC\\src\\drpc_gateway.inc");
        const std::string discordMain = ReadSource("..\\DiscordRPC\\DiscordRPC.cpp");
        ...
        checks.push_back({"DiscordRPC uses shared JSON primitives", "DiscordRPC JSON sources", sharedCore + "\n" + discordCore, R"rx(inline std::string JsonEscape.*inline size_t JsonStringEnd.*inline bool FindJsonFieldValue.*using aip::ExtractJsonStringValue.*using aip::FindJsonFieldValue.*using aip::JsonStringEnd.*using aip::JsonValueEnd)rx", "DiscordRPC must consume the shared JSON builder, scanner, and string decoder implementations", false});
        checks.push_back({"DiscordRPC does not shadow shared JSON primitives", "DiscordRPC/src/drpc_core.inc", discordCore, R"rx(static std::string JsonEscape|static void JsonFieldRaw|static size_t JsonStringEnd|static size_t JsonValueEnd|static bool FindJsonFieldValue|static std::wstring ExtractJsonStringValue)rx", "DiscordRPC must not maintain local copies that shadow shared JSON fixes", true});
        checks.push_back({"Shared command-line dependency provides baseline primitives", "..\\dependencies\\command_line.inc", sharedCommandLine, R"rx(struct IniSetting.*enum class IniSetSpecError.*IsOptionOrInlineValue.*TakeCommandLineValue.*ParseIniSetSpec.*ParseBoolValue)rx", "dependencies/command_line.inc must provide the shared setting type, option-value extraction, INI override parsing, and boolean parsing used by baseline projects", false});
        ...
        checks.push_back({"DiscordRPC uses shared command-line setting primitives", "DiscordRPC command-line sources", discordTypes + "\n" + discordCommandLine, R"rx(using CommandLineSetting = aip::IniSetting.*aip::ParseIniSetSpec)rx", "DiscordRPC must use the same shared INI override setting representation as DesktopStub", false});
        ...
        checks.push_back({"Shared application baseline exposes explicit subsystem contracts", "..\\dependencies\\baseline_app.h", baselineApp, R"rx(struct InstanceIdentity.*class ResidentShutdownState.*Request\(\).*Cancel\(\).*MarkWorkComplete\(\).*BuildInstanceIdentity.*SignalInstanceWindow.*RegisterTaskbarCreatedMessage.*FormatTextTemplate.*class TraySectionLayout)rx", "the baseline must expose reusable resident shutdown, single-instance, taskbar, help-template, and tray-layout APIs instead of product-local copies", false});
        checks.push_back({"Shared desktop baseline codifies module order", "..\\dependencies\\desktop_app_baseline.h", desktopAppBaseline, R"rx(#include "baseline_app\.h".*#include "core\.inc".*#include "config_ini\.inc".*#include "command_line\.inc".*#include "tray\.inc")rx", "baseline consumers must have one stable include entry point instead of reproducing shared module ordering", false});
        checks.push_back({"Shared config dependency exposes an explicit config store", "..\\dependencies\\config_ini.inc", sharedConfig, R"rx(class IniConfigStore.*ReadRaw.*WriteRaw.*EnsureDefaults.*ReadFreshText.*MutateFresh.*IniWriteMutexGuard guard\(path_\).*WriteFreshText)rx", "baseline projects must have a reusable configuration store with atomic fresh read-modify-write transactions", false});
        checks.push_back({"DesktopStub consumes shared application subsystems", "DesktopStub baseline sources", desktopStub + "\n" + app + "\n" + commandLine + "\n" + tray, R"rx(dependencies\\desktop_app_baseline\.h.*aip::BuildInstanceIdentity.*aip::SignalInstanceWindow.*aip::RegisterTaskbarCreatedMessage.*aip::FormatTextTemplate.*aip::TraySectionLayout)rx", "DesktopStub must be a consumer of the reusable baseline rather than the source of private implementations", false});
        checks.push_back({"DiscordRPC consumes shared application subsystems", "DiscordRPC baseline sources", discordMain + "\n" + discordCore + "\n" + discordApp + "\n" + discordDefaults + "\n" + discordTray, R"rx(dependencies\\desktop_app_baseline\.h.*aip::IniConfigStore.*aip::BuildInstanceIdentity.*aip::SignalInstanceWindow.*aip::RegisterTaskbarCreatedMessage.*aip::FormatTextTemplate.*aip::TraySectionLayout)rx", "DiscordRPC must use the same application subsystem contracts as DesktopStub", false});
        ...
        checks.push_back({"DiscordRPC matches the baseline configurable tray layout", "DiscordRPC tray/config sources", discordDefaults + "\n" + discordTray, R"rx(show_menu_as_dropdown.*menu_dropdown.*showMenuAsDropdown.*beginSection.*endSection.*ID_TOGGLE_MENU_DROPDOWN)rx", "DiscordRPC must support the same flat or dropdown tray section model as DesktopStub", false});
        checks.push_back({"DiscordRPC command-line help is INI-backed and side-effect-free", "DiscordRPC help/startup sources", discordDefaults + "\n" + discordMain, R"rx(DEFAULT_HELP_TEMPLATE.*IniConfigStore\(g_iniPath\)\.ReadRaw\(.*aip::FormatTextTemplate.*if \(options\.showHelp\).*PrintLine\(DefaultHelpText\(\)\).*EnsureDefaultConfigFile)rx", "DiscordRPC help must read an existing configurable template before any default-file creation", false});
        checks.push_back({"DiscordRPC restores its tray icon after Explorer restarts", "shared/DiscordRPC app/tray sources", baselineApp + "\n" + discordApp + "\n" + discordTray, R"rx(RegisterWindowMessageW\(L"TaskbarCreated"\).*aip::RegisterTaskbarCreatedMessage\(\).*g_taskbarCreatedMessage.*ctx->trayEnabled.*ctx->trayCreated = false.*CreateTrayIcon\(ctx\))rx", "resident tray applications must recreate their notification icon after the taskbar is rebuilt", false});
        checks.push_back({"DiscordRPC worker notifications are marshalled to the window thread", "DiscordRPC app/tray/type sources", discordTypes + "\n" + discordApp + "\n" + discordTray, R"rx(notificationMutex.*pendingNotifications.*QueueTrayBalloon.*PostMessageW\(ctx->hwnd,\s*WM_DRPC_NOTIFY.*case WM_DRPC_NOTIFY:.*ShowPendingTrayBalloons)rx", "worker threads must not mutate NOTIFYICONDATAW or tray lifecycle state directly", false});
        checks.push_back({"DiscordRPC resident mode requires a control window", "DiscordRPC/src/drpc_app.inc", discordApp, R"rx(if \(hwnd == nullptr\).*if \(options\.once\).*RunPresenceLoop\(true,\s*nullptr\).*return 1;.*try\s*\{\s*ctx\.worker = std::thread.*catch \(const std::exception& ex\).*DestroyWindow\(hwnd\);\s*return 1;)rx", "resident mode must fail instead of holding the single-instance mutex without a signalable control window, and worker startup failures must be contained", false});
        checks.push_back({"Resident projects share the baseline shutdown state", "DesktopStub/DiscordRPC shutdown sources", baselineApp + "\n" + defaults + "\n" + generation + "\n" + discordTypes + "\n" + discordApp + "\n" + discordTray, R"rx(class ResidentShutdownState.*static aip::ResidentShutdownState g_shutdownState.*g_shutdownState\.Request\(\).*g_shutdownState\.Cancel\(\).*aip::ResidentShutdownState shutdown.*WM_DRPC_WORKER_FINISHED.*ID_TRAY_FORCE_EXIT.*RequestDiscordShutdown.*force_shutdown_pending_cleanup)rx", "DesktopStub and DiscordRPC must use the same resident shutdown state while retaining product-specific cleanup behavior", false});
        checks.push_back({"DiscordRPC backend waits are interruptible", "DiscordRPC backend/app sources", discordApp + "\n" + discordIpc + "\n" + discordGateway, R"rx(static bool BackendControlRequested\(\).*DiscordIpcClient\(clientId,\s*verbose,\s*LoadIpcOptions\(\),\s*BackendControlRequested\).*DiscordGatewayClient\(token,\s*clientId,\s*verbose,\s*LoadGatewayOptions\(\),\s*BackendControlRequested\).*WaitForPipe\(.*std::min<DWORD>\(remaining,\s*50\).*RunCancellableWinHttpRequest\(.*Gateway connection cancelled.*WinHttpCloseHandle\(request\))rx", "resident exit, reload, and refresh must actively interrupt IPC and HTTP waits instead of waiting for their full configured timeout", false});
        checks.push_back({"DiscordRPC applies the configured Gateway close timeout", "DiscordRPC Gateway sources", discordCore + "\n" + discordGateway, R"rx(closeTimeoutMs = ClampInt\(IniReadI\(L"gateway",\s*L"close_timeout_ms".*WINHTTP_OPTION_WEB_SOCKET_CLOSE_TIMEOUT.*WinHttpWebSocketClose)rx", "the configured Gateway close timeout must control the WinHTTP WebSocket close handshake", false});
        checks.push_back({"Windows smoke covers DiscordRPC resident control", "..\\.github\\tools\\RepoTools.cs", repoTools, R"rx(VerifyDiscordRpcNoTrayControl.*DiscordRPC no-tray reload.*Configuration reloaded\..*DiscordRPC no-tray exit.*DiscordRPC missing resident exit)rx", "smoke tests must exercise reload and exit delivery for a real no-tray resident process", false});
        ...
        checks.push_back({"Effective INI uses separate single-instance scope", "src\\ga_app.inc", app, R"rx(ConfigureSingleInstanceIdentity\(const std::wstring& exeDir,\s*const std::wstring& iniPath,\s*bool customIniPath\).*Scope by the effective INI path.*!iniPath\.empty\(\)\s*\?\s*iniPath\s*:\s*exeDir.*aip::BuildInstanceIdentity.*ConfigureSingleInstanceIdentity\(dir,\s*g_iniPath,\s*g_commandLine\.customIniPath\))rx", "renamed EXE/default INI and alternate --ini runs must not signal each other unless they share the same INI", false});
