...
int main(int argc, char** argv)
{
    ...
    try
    {
        ...
        checks.push_back({"Build script derives DesktopStub version from CI release tags", "BuildDesktopStub.cmd", buildScript, R"rx(ResolveDesktopStubVersion.*DESKTOPSTUB_VERSION.*DESKTOPSTUB_RELEASE_TAG.*git tag --points-at HEAD --list DesktopStub-v\*.*git tag --list DesktopStub-v\*.*DesktopStub-v%NEXT_DESKTOPSTUB_TAG_NUMBER%)rx", "BuildDesktopStub.cmd must derive the default DesktopStub version from the DesktopStub-vN CI release family while allowing explicit overrides", false});
        checks.push_back({"Build script embeds DesktopStub version resources", "BuildDesktopStub.cmd", buildScript, R"rx(RC_VERSION_DEFINES.*rc /nologo %RC_VERSION_DEFINES% /fo "%BROKER_RES%" LiveTileBroker\.rc.*cl .*%VERSION_DEFINES% LiveTileBroker\.cpp "%BROKER_RES%".*rc /nologo %RC_VERSION_DEFINES% /fo "%RES_FILE%" DesktopStub\.rc.*cl .*%VERSION_DEFINES% DesktopStub\.cpp "%RES_FILE%")rx", "BuildDesktopStub.cmd must compile version resources into both DesktopStub executables", false});
        ...
        checks.push_back({"INI template does not expose manifest editor defaults", "src\\ga_config_defaults.inc", defaults, R"rx(BuildInitialIniTemplate(?!.*lines\.push_back\(L"\[Manifest\]"\)))rx", "INI template must not recreate the redundant [Manifest] editor section", false});
