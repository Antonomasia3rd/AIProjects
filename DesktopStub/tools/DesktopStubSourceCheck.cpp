...
int main(int argc, char** argv)
{
    ...
    try
    {
        ...
        checks.push_back({"DesktopStub mutations bypass the timestamp cache", "src\\ga_core.inc", core, R"rx(static bool ReadIniFileForMutation\(std::wstring& text\)\s*\{\s*DWORD attrs = GetFileAttributesW\(g_iniPath\.c_str\(\)\).*\}\s*static bool ReadIniValueCached)rx", "INI mutations must reread disk after taking the mutation lock so preserved or coalesced timestamps cannot overwrite external edits", false});
        ...
        checks.push_back({"DiscordRPC command-line help is INI-backed and side-effect-free", "DiscordRPC help/startup sources", discordDefaults + "\n" + discordMain, R"rx(DEFAULT_HELP_TEMPLATE.*IniReadS\(L"CommandLineHelp",\s*L"Template".*ReplaceHelpToken.*if \(options\.showHelp\).*PrintLine\(DefaultHelpText\(\)\).*EnsureDefaultConfigFile)rx", "DiscordRPC help must read an existing configurable template before any default-file creation", false});
        checks.push_back({"DiscordRPC restores its tray icon after Explorer restarts", "DiscordRPC app/tray sources", discordApp + "\n" + discordTray, R"rx(RegisterWindowMessageW\(L"TaskbarCreated"\).*g_taskbarCreatedMessage.*ctx->trayEnabled.*ctx->trayCreated = false.*CreateTrayIcon\(ctx\))rx", "resident tray applications must recreate their notification icon after the taskbar is rebuilt", false});
