...
int main(int argc, char** argv)
{
    ...
    try
    {
        ...
        checks.push_back({"Local builds refresh release tags before versioning", "BuildDesktopStub.cmd", buildScript, R"rx(if not defined DESKTOPSTUB_VERSION if not defined DESKTOPSTUB_RELEASE_TAG call :RefreshDesktopStubTags.*:RefreshDesktopStubTags.*DESKTOPSTUB_SKIP_TAG_REFRESH.*git remote get-url origin.*git fetch --quiet --force --tags --prune --prune-tags origin.*continuing with local tags.*:ResolveDesktopStubVersion)rx", "default local builds must not reuse a release number merely because their tag refs are stale, while offline and explicit-version builds remain supported", false});
