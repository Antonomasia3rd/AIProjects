...
static const StringDefault g_stringDefaults[] = {
    ...
    {L"DesktopIconFallbackSource", L"Desktop icon fallback source: %s"},
    {L"DesktopIconFallbackMissing", L"Desktop icon fallback source not found; skipping %s."},
...
struct UiStrings
{
    ...
    std::wstring desktopIconFallbackSource;
    std::wstring desktopIconFallbackMissing;
...
void LoadUiStrings()
{
    ...
    g_ui.desktopIconFallbackSource = IniReadS(L"Strings", L"DesktopIconFallbackSource", L"Desktop icon fallback source: %s");
    g_ui.desktopIconFallbackMissing = IniReadS(L"Strings", L"DesktopIconFallbackMissing", L"Desktop icon fallback source not found; skipping %s.");
...
static void ValidateFormatStrings()
{
    ...
    RequireFormat(g_ui.desktopIconFallbackSource, L"DesktopIconFallbackSource", { L"%s" });
    RequireFormat(g_ui.desktopIconFallbackMissing, L"DesktopIconFallbackMissing", { L"%s" });
...
static std::vector<int> GetConfiguredScales()
{
    ...
    if (g_generateScaleAuto) AddScale(scales, CurrentAssetScale());
...
static std::wstring FindDesktopIconFallbackPath(const std::wstring& exeDir)
{
    static const wchar_t* candidates[] = {
        L"Assets\\SmallTile.png",
        L"Assets\\Square44x44Logo.png",
        L"Assets\\StoreLogo.png"
    };

    for (const wchar_t* file : candidates)
    {
        std::wstring path = exeDir + L"\\" + file;
        if (PathFileExistsW(path.c_str()))
        {
            Log(g_ui.desktopIconFallbackSource.c_str(), path.c_str());
            return path;
        }
    }
...
bool Generate(const wchar_t* wp, const wchar_t* reason = nullptr)
{
    ...
    bool assetsOk = true;
    std::wstring desktopIconFallbackPath;
    ...
    for (auto& t : g_tiles)
    {
        ...
        if (!IniReadI(L"Assets", t.name, 0))
        {
            ...
            if (desktopIconFallbackPath.empty())
                desktopIconFallbackPath = FindDesktopIconFallbackPath(exeDir);

            if (desktopIconFallbackPath.empty())
            {
                assetsOk = false;
                Log(g_ui.desktopIconFallbackMissing.c_str(), t.name);
                continue;
            }

            bool copied = IEquals(desktopIconFallbackPath, outPath) || CopyFileW(desktopIconFallbackPath.c_str(), outPath.c_str(), FALSE);
            for (int scale : scales)
            {
                std::wstring scaledPath = ScaleAssetPath(outPath, scale);
                copied = copied && (IEquals(desktopIconFallbackPath, scaledPath) || CopyFileW(desktopIconFallbackPath.c_str(), scaledPath.c_str(), FALSE));
            }
        ...
        for (int scale : scales)
        {
            ...
            if (scaled)
            {
                saved = SavePNG(scaled, scaledPath.c_str()) && saved;
                delete scaled;
            }
            else
            {
...
void Menu(HWND h)
{
    ...
    addTo(scaleMenu, ID_SCALE_100, g_ui.generateScale100.c_str(), g_generateScale100);
    addTo(scaleMenu, ID_SCALE_125, g_ui.generateScale125.c_str(), g_generateScale125);
    addTo(scaleMenu, ID_SCALE_150, g_ui.generateScale150.c_str(), g_generateScale150);
    addTo(scaleMenu, ID_SCALE_200, g_ui.generateScale200.c_str(), g_generateScale200);
    addTo(scaleMenu, ID_SCALE_400, g_ui.generateScale400.c_str(), g_generateScale400);
