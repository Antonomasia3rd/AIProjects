...
echo ERROR: MSVC and the Windows SDK are required for HTTP tests.
exit /b 1
:LoadCompiler
call "%VCVARS%" >nul
...
cl /nologo /utf-8 /std:c++17 /EHsc /W4 /DUNICODE /D_UNICODE tools\HttpFetchTests.cpp /Fe:build\HttpFetchTests.exe /Fo:build\HttpFetchTests.obj /link winhttp.lib ws2_32.lib /SUBSYSTEM:CONSOLE
set "STATUS=%ERRORLEVEL%"
if not "%STATUS%"=="0" goto Done
call build\HttpFetchTests.exe
set "STATUS=%ERRORLEVEL%"
:Done
popd
exit /b %STATUS%
