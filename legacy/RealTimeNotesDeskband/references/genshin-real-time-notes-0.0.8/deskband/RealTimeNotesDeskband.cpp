...
static const ResourceMetadata kResources[] = {
    {
        ...
        L"embedded\\assets\\genshin\\resin_full.ico",
        L"embedded\\assets\\genshin\\resin_not_full.ico",
        L"embedded\\assets\\genshin\\resin_error.ico",
    ...
    },
    {
        ...
        L"Stamina",
        L"hsr_cookie.json",
        L"embedded\\assets\\hsr\\stamina_full.ico",
        L"embedded\\assets\\hsr\\stamina_not_full.ico",
        L"embedded\\assets\\hsr\\stamina_error.ico",
        L"bbs-api-os.hoyolab.com",
    ...
    },
    {
        ...
        L"Charge",
        L"zzz_cookie.json",
        L"embedded\\assets\\zzz\\charge_full.ico",
        L"embedded\\assets\\zzz\\charge_not_full.ico",
        L"embedded\\assets\\zzz\\charge_error.ico",
...
static ResourceKind ReadResourceSetting(const std::wstring& installDir)
{
    ...
    for (const auto& item : kResources)
    {
        if (FileExists(JoinPath(installDir, item.configFile)))
        {
            return item.resource;
        }
    }
...
static NoteState FetchState(const std::wstring& installDir)
{
    ResourceKind resource = ReadResourceSetting(installDir);
    const auto& metadata = MetadataFor(resource);

    AppConfig cfg;
    if (!LoadConfig(JoinPath(installDir, metadata.configFile), cfg))
    {
        std::wstring line2 = L"Run ";
        ...
        return ErrorState(resource, L"Login needed", line2);
    }
...
private:
    ...
    static DWORD WINAPI RefreshThread(void* context)
    {
        ...
        NoteState state = FetchState(band->m_installDir);
    ...
    std::wstring IconPathFor(const NoteState& state)
    {
        ...
        const wchar_t* relative = metadata.notFullIcon;
        ...
        return JoinPath(m_installDir, relative);
    }
    ...
    void OnPaint()
    {
        ...
        if (icon)
        {
            DrawIconEx(hdc, rc.left + margin, iconY, icon, iconSize, iconSize, 0, nullptr, DI_NORMAL);
            DestroyIcon(icon);
        }
